...
if selected == 'Earthquake Prediction':
    ...
    if st.button('Earthquake Result'):
        ...
        magnitude = Earthquake_model.predict([user_input])


        if magnitude < 0:
            st.success("No sign of Earthquake")
        elif magnitude > 0 and magnitude < 2.0:
            st.success("Minor Earthquake")
        elif magnitude >= 2.0 and magnitude < 4.0:
            st.success("Light Earthquake")
        elif magnitude >= 4.0 and magnitude < 6.0:
            st.success("Moderate Earthquake")
        elif magnitude >= 6.0 and magnitude < 8.0:
            st.success("Strong Earthquake")
        else:
            st.success("Major Earthquake")

        st.success(magnitude)
...
if selected == 'Drought Prediction':
    ...
    if st.button('Drought probability Result'):
        try:
            ...
            drought_diagnosis = f"level of drought: {drought_prediction[0]}"
            if drought_prediction == 0:
                st.success("Abnormly dry")

            elif drought_prediction == 1:
                st.success("MOderate Drought")
            elif drought_prediction == 2:
                st.success("Severe Drought")
            elif drought_prediction == 3:
                st.success("Extreme Drought")
            else:
                st.success("Exceptional Drought")
        except ValueError as ve:
